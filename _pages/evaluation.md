...
**Subtask 1** will be evaluated in two ways as already experimented in the Le.Wi.Di shared tasks at
SemEval 2021 and 2023 about learning with disagreement (Uma et al. 2021 and Leonardelli et al. 2023).
First, by using the standard classification metric **F1** after solving the disagreement cases in the
gold standard. The second evaluation metric will be the **cross-entropy** between the system output
values and the soft labels generated by a probabilistic normalization procedure.
...
- The **ICM metric** (Amigó and Delgado, 2022), which is an information theoretic based metric which
  considers both the hierarchical structure and the class specificity. It is applicable to both hard and
  soft labels. **The ICM metric will be the official metric considered for the ranking for both hard and soft labels.**
{: style="text-align: justify;"}
...
Teams will be allowed (and encouraged) to submit multiple runs (max. 5). for each subtask.
{: style="text-align: justify;"}
